...
各 run の `research/runs/<ts>/run_notes.md` に、その run の実行結果と気づきを記録する。**手順書通りに進まなかったこと・予想外のこと・各段階の件数概要を残す目的**。本手順書の出力ファイル共通原則と同様に、書き出した内容は不変扱いとする (誤りが見つかったら別 run の `run_notes.md` で訂正する)。
...
1. **この run の位置づけ**: run_id (`<ts>`)、目的、対象データ、進捗状況、前 run との関係
2. **件数フロー実績**: 各段階の入力 → 出力件数の表 (前 run との比較を含めると尚良)
...
7. **ファイル参照クイックリンク**: 主要出力ファイル (生データ・主要中間出力・最終 CSV) へのリンク
...
[`research/runs/2026_05_15_10_00/run_notes.md`](../research/runs/2026_05_15_10_00/run_notes.md) (本手順書の通し実行 run)
