...
例: [`../reports/2026/04/2026_04_24_02_メルカリ売れ筋リサーチ_v2.csv`](../reports/2026/04/2026_04_24_02_メルカリ売れ筋リサーチ_v2.csv) の `maさま` クラスタ (`ネックレス_チェーンネックレス_019`、3 件、仕入れ候補フラグあり) — タイトルが `maさま` のみで商品情報ゼロだが、画像判定で 3 件が正しく 1 クラスタに集約された。仕入れ判断者は代表 URL 3 件を画像で確認して判断できるため、タイトルの情報希薄性は仕入れ判断の障害でなくなった。
...
| 2026-04-18 | 廃止 | 仕入れ判断者「取引専用でも第三者購入できるケースあり、除外不要」 |
